...
class ConvolutionalNeuralNetwork(_ConvNetBase):
	...
	Args:
		...
		*layer_params: convolutional layers specifications in order.
	"""
	def __init__(self, *layer_params, activation=None, input_shape=None):
		assert not activation is None, "activation must be defined" 
		assert not input_shape is None, "input_shape must be defined" 
		self._shape_before_flatten = None
		layer_params = list(layer_params)
		assert self.is_layers_valid(layer_params), "parameters specified do not match requirements for object." 
		conv_layer_params, ff_layer_params = self.separate_ff_and_conv(layer_params)
		super().__init__(activation, *layer_params)
		assert list(self._layer_params) == conv_layer_params+ff_layer_params, "conv and resnet layers parameters must be first."
		...
		conv_layer = [tf.keras.layers.InputLayer(input_shape)]
		for i in range(len(conv_layer_params)):
			#setup parameters
			layer_p, up_param = self.separate_upscale_or_pooling_parameter(conv_layer_params[i])
		...
		ff_layer = []
		for i in range(len(ff_layer_params)):
			# setup parameters
			layer_num = len(conv_layer_params)+i
			...
			# get the ff layers 
			ff_layer+=self.create_ff_layers(ff_layer_params[i], layer_num)
...
class DeconvolutionalNeuralNetwork(_ConvNetBase):
	...
	Examples:
		...
		>>> a = DeconvolutionalNeuralNetwork(*layer_params, 
		>>> 	activation=activations, shape_before_flatten=[8,8,256])
		>>> a = tf.keras.Sequential([a])
		>>> print(a(inputs).shape) # the model must be run for keras to collect the trainable variables/weights
		>>> print(len(a.weights))
	...
	def __init__(self, *layer_params, activation=None, shape_before_flatten=None):
		...
		conv_layer_params, ff_layer_params = self.separate_ff_and_conv(layer_params)
		super().__init__(activation, *layer_params)
		assert list(self._layer_params) == ff_layer_params + conv_layer_params, "conv and resnet layers parameters must be first."
			#make sure that the layers have been defined in order.
			#if you want to automate the layers to rearrange themselves, make sure to 
			#take care of the activations as well.
		...
		for i in range(len(ff_layer_params)):
			# get the ff layers 
			ff_layer+=self.create_ff_layers(ff_layer_params[i], i)
		...
		for i in range(len(conv_layer_params)):
			...
			#setup parameters
			layer_p, up_param = self.separate_upscale_or_pooling_parameter(conv_layer_params[i])
			layer_num = len(ff_layer_params)+i
			
			if not i:
				prod_reshape = reduce(lambda x,y: x*y, shape_before_flatten)
